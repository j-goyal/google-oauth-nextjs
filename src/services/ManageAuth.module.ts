...
export const ManageAuth = () => {
  ...
  const getCurrentUser = () => axios.getData("/api/v1/auth/me");

  const deleteCurrentUser = () => axios.deleteData("/api/v1/auth/me");

  const confirmRestoreUser = (payload: { email: string }) =>
    axios.postData("/api/v1/auth/google/confirm-restore", payload);

  const logoutCurrentSession = () => axios.postData("/api/v1/auth/logout");

  const logoutAllSessions = () => axios.postData("/api/v1/auth/logout-all");

  const logoutOtherSessions = () =>
    axios.postData("/api/v1/auth/logout-others");
